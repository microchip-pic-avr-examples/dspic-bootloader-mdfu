...
_Figure 2. - dsPIC33AK256MPS306 Curiosity GP DIM_<br><br>
...
* dsPIC33AK-MP_DFP (Device Family Pack) 1.3.185 or later
...
* [Cryptographic Accelerator Module (CAM) driver for dsPIC33AK256MPS306 version 1.4.0](https://github.com/microchip-pic-avr-examples/dspic33a-crypto-accelerator-module)
...
### Hardware
* [dsPIC33AK256MPS306 DIM (EV02E57A)](https://www.microchip.com/en-us/development-tool/EV02E57A)
...
### Hardware Setup
1. Insert the dsPIC33AK256MPS306 Curiosity GP Dim into the dsPIC33A Curiosity DIM Connector.
...
* Hexmate: An application included with MPLAB X IDE 6.25 and later and the MPLAB XC8 Compiler, to merge, reformat, and insert data into hex files. Documentation for Hexmate can be found in the XC8 installation directory on your PC (e.g., Program Files → Microchip → xc8 → v3.00 → docs).
...
2. In the command terminal, enter the following command to begin the firmware update:<br>
    ```
     pymdfu update --tool serial --image dist/default/production/signed_image.bin --port COMXX --baudrate 460800
    ```   
    <br>**NOTE**: Replace "COMXX" with the correct communication port used in your setup. For example:</br>
    ```
     pymdfu update --tool serial --image dist/default/production/signed_image.bin --port COM12 --baudrate 460800
    ```
    ...
    <br>**NOTE**: "--verbose debug" may be added in the command above to show a detailed description of the packets being transferred. Additional details on the pymdfu command usage can be found [here](https://pypi.org/project/pymdfu/).</br>
...
| ------------------------------------------- | ------------ | ------------------------------------ | ---------------------------- | --------------------------------------------------------------------------------------------------------------------- | 
| MDFU_PARTITION_HEADER_CODE_SIZE           | 0x00000001UL | 4                                    | 0x00011BC0                  | Number of bytes in the executable image                                                                              |
...
## ML-DSA Hardware Acceleration Performance
This demo includes a dsPIC33A CAM-accelerated ML-DSA-87 verification path to improve throughput compared to the standard software-only wolfCrypt implementation.

For the evaluated ML-DSA-87 verification configuration, the CAM-accelerated implementation reduced verification time from 46.378 ms to 11.683 ms when built with `-Os`, corresponding to a reduction of approximately 75% compared to software alone.

The example projects in this demo are distributed with `-O1` optimization enabled rather than `-Os`. In that default demo configuration, ML-DSA-87 verification is approximately 15 ms.

These values are provided as example benchmark data for the ML-DSA verification operation itself and should not be interpreted as the total end-to-end boot or firmware update time improvement for this demo. Actual system-level improvement depends on how much of the overall boot or update flow is spent in ML-DSA processing.

For this demo, the accelerated path is enabled through the wolfCrypt and project configuration updates described in the [Project Properties Modifications](#project-properties-modifications) section.

## Linker File, Project Properties, and MCC Library Modifications
The linker files (p33AK256MPS306.gld) as well as a handful of project properties and library files in both boot.X and app.X have been modified for this project. The following sections contain descriptions of those changes and considerations to make when porting this demo to another device within the dsPIC33A family.

### Linker File Modifications 
The linker files for boot.X and app.X can be found in the root directory of each respective project and are both named "p33AK256MPS306.gld". For comparison, the default linker files can be found in the device packs folder. Typically this will be under "C:\Users\XXXXX\.mchp_packs\Microchip\dsPIC33AK-MP_DFP\X.X.X\xc16\support\dsPIC33A\gld".
...
##### wolfCrypt Modifications for Accelerated ML-DSA
This project includes additional wolfCrypt integration changes required for the accelerated ML-DSA-87 verification flow on dsPIC33A devices that support SHAKE128 and SHAKE256 hardware acceleration. If porting this demo to a new project, the standard wolfCrypt source files alone are not sufficient.

* The following dsPIC33A CAM hash support files must be included in the project:
    * `dspic33a_cam_hash.c`
    * `dspic33a_cam_hash.h`
* In addition to the files above, the following Common Crypto support directories must also be included:
...
The following wolfCrypt source files require updates to support the accelerated ML-DSA implementation:

* `crypto/wolfssl/wolfssl/wolfcrypt/sha3.h`

    Add the following include when `dsPIC33A_CAM_ENABLE` is defined:
    ```c
    ...
    Also add the following members to the SHA3 structure when `dsPIC33A_CAM_ENABLE` is defined:
    ```c
...
* `crypto/wolfssl/wolfcrypt/src/sha3.c`

    Update the SHA3 software implementation guard so the software SHA3 path is excluded when `dsPIC33A_CAM_ENABLE` is defined.
...
* `crypto/wolfssl/wolfcrypt/src/dilithium.c`

    Before each relevant `wc_InitShake256(shake256, NULL, INVALID_DEVID);` call in the ML-DSA flow, add:
    ```c
    #ifdef dsPIC33A_CAM_ENABLE
    ...
    This update is required so the SHAKE digest length is available to the dsPIC33A CAM-backed implementation.
...
As shown in _Figure 23_ the executable region resides in address range 0x81A000-0x82CFFF (inclusive).  To configure PR2 to this range, we configure the PR2ST register to 0x1A and the PR2END register to 0x2C.  *NOTE* - the PRnST and PRnEND registers are offset from 0x800000 and are page aligned (the lower bits are hardware restricted to force page alignment).  These options are set in the configuration bits FPR2ST and FPR2END which are copied into the PR2ST and PR2END SFRs on reset.
...
As shown in _Figure 23_ the download region resides in address range 0x82D000-0x83FFFF (inclusive).  To configure PR3 to this range, we configure the PR3ST register to 0x2D and the PR3END to 0x3F.  *NOTE* - the PRnST and PRnEND registers are offset from 0x800000 and are page aligned (the lower bits are hardware restricted to force page alignment).  These options are set in the configuration bits FPR3ST and FPR3END which are copied into the PR3ST and PR3END SFRs on reset.
