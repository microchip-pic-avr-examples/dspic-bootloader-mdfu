...
bool MDFU_INTERNAL_FLASH_BlockErase(uint32_t startAddress, uint32_t length)
{
    ...
    if((startAddress % FLASH_ERASE_PAGE_SIZE_IN_BYTES) != 0UL){
        ...
        for(uint32_t eraseAddress = startAddress; eraseAddress < endAddress; eraseAddress += FLASH_ERASE_PAGE_SIZE_IN_BYTES)
        {
            ...
            enum FLASH_RETURN_STATUS eraseStatus = FLASH_PageErase(physicalEraseAddress, FLASH_UNLOCK_KEY);
            CRITICAL_REGION_End();
            if (eraseStatus != FLASH_NO_ERROR)
            {
                status = false;
                break;
            }
...
bool MDFU_INTERNAL_FLASH_Write(uint32_t address, size_t length, uint8_t const * const data)
{
    ...
    if(NULL == data){
        ...
        while(count < length)
        {
            ...
            CRITICAL_REGION_Begin();
            enum FLASH_RETURN_STATUS wordWriteStatus = FLASH_WordWrite(address+count, flashData, FLASH_UNLOCK_KEY);
            CRITICAL_REGION_End();
            if (wordWriteStatus != FLASH_NO_ERROR)
            {
                status = false;
                break;
            }
