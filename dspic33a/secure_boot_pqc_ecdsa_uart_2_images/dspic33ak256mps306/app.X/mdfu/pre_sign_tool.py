...
def generate_hex_and_bin(section_cfgs, filled_hex, hex_out, bin_out, obj_cpy, dfp):
    """
    ...
    Args:
        ...
        dfp (str): Path to DFP used for project.
    ...
    run_cmd(f'"{obj_cpy}" -I ihex -O binary "{hex_out}" "{bin_out}" -mdfp="{dfp}"')

def generate_binary(exec_start, exec_end, filled_hex, presigned_hex, signed_bin, obj_cpy, dfp):
    """
    ...
    run_cmd(f'"{obj_cpy}" -I ihex -O binary "{presigned_hex}" "{signed_bin}" -mdfp="{dfp}"')
...
def main():
    ...
    parser.add_argument('--dfp', required=True, help='Path to DFP used for project.')
    ...
    generate_hex_and_bin(cfg["executable_header"], paths["filled_hex"], paths["header_hex"], paths["header_bin"], obj_cpy, dfp)
    ...
    generate_hex_and_bin(sections, paths["filled_hex"], paths["data_hex"], paths["data_bin"], obj_cpy, dfp)

    # Generate presigned_image.hex and signed_image.bin (full executable region)
    generate_binary(exec_start, exec_end, paths["filled_hex"], paths["presigned_hex"], paths["signed_bin"], obj_cpy, dfp)
