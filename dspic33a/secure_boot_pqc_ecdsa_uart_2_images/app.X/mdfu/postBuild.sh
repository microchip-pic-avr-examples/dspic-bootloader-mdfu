...
export mldsa87_private_key_path="../../boot.X/mdfu/keystore/mldsa87_private_key.pem"
...
# Check that the mldsa87 private key is present
if ! [ -e $mldsa87_private_key_path ]; then
    MissingPrivateKey
fi
...
openssl dgst -sha512 "$projectDir/$imageDir/data.bin" || Error
openssl dgst -sha512 -binary -out "$projectDir/$imageDir/data.hash.bin" "$projectDir/$imageDir/data.bin" || Error
...
echo ..............................
# Sign application header binary file
openssl pkeyutl -sign -inkey $mldsa87_private_key_path -rawin -in "$projectDir/$imageDir/header.bin" -out "$projectDir/$imageDir/mldsa87_signature.bin" || Error
