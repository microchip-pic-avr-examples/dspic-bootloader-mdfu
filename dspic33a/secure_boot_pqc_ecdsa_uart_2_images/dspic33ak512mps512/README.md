...
* Hybrid secure boot authentication, where the code is hashed using SHA\-512, followed by ECDSA P\-384 signing and verification with the Crypto Accelerator Module (CAM) hardware, and ML-DSA-87 signing and verification performed by wolfSSL's wolfCrypt library using the standard software implementation.
...
* [WolfCrypt](https://github.com/wolfSSL/Microchip)
    ...
    * [WolfCrypt](https://github.com/wolfSSL/Microchip)
    * **NOTE**: See boot.X for the wolfSSL License, EULA, and version information used for demo.
    * **NOTE**: In this demo, ML-DSA-87 uses the standard wolfCrypt software implementation. A separate dsPIC33A demo is available to illustrate an accelerated ML-DSA integration for supported hardware configurations.
...
* A post build step was added under Project Properties &rarr; Building &rarr; Execute this line after build. This box should be checked and the following should be added if porting this to a new project:
    ```
    cd mcc_generated_files/boot && ${/}postBuild$(ShExtension) $(MP_CC_DIR) ${ProjectDir} ${ImageDir} ${ImageName} ${IsDebug} ${DFP_DIR} && cd ${ProjectDir} && cd ${ImageDir}
    ```
...
## ML-DSA Implementation Notes
In this demo, ML-DSA-87 signing and verification are performed using the standard wolfCrypt software implementation. ECDSA P-384 operations in this demo use the dsPIC33A Crypto Accelerator Module (CAM) hardware. However, the additional dsPIC33A CAM-based integration that can be used to accelerate ML-DSA on some supported hardware configurations is not used in this project. As a result, the additional CAM-specific wolfCrypt source changes, support files, and build settings used by accelerated ML-DSA examples are not required for this dsPIC33AK512MPS512 demo.

Explore other demo projects in this folder for examples of devices that have PQC hardware acceleration and the associated improvement on image verification time.
