...
* Secure firmware update via CAN-FD using the Microchip Device Firmware Update (MDFU) protocol.
...
* The drivers for the selected CAN-FD protocol analyzer/protocol generator
...
* A CAN protocol analyzer/protocol generator. A [PCAN-USB FD](https://www.peak-system.com/PCAN-USB-FD.365.0.html?&L=1) was used for development of this demo
...
3. Connect the J21 4-pin terminal header to the CAN protocol analyzer/protocol generator.<br>
![CAN FD Driver](./images/canFDDriver.jpg)<br>
    _Figure 3. - CAN FD Driver_<br>
4. Connect the CAN protocol analyzer to the host computer. <br>
...
2. Press the "Make and Program" button on the top bar, selecting the appropriate programmer if prompted.<br>
    ...
    _Figure 4. - "Make and Program" button in MPLAB X_<br>
...
<br>
The script contains two main python scripts:<br>
...
2. Click the dropdown arrow next to the “Clean and Build Project” button on the top bar and select "Clean and Build Main Project". <br>
    ...
    _Figure 8. - "Clean and Build Project" button in MPLAB X_<br>
...
Below is a step-by-step description of the commands executed in the postBuild.bat/.sh script:
...
4. Generate a binary file of the whole executable space using Hexmate. The first command extracts the executable code portion of the .hex file created in Step 1 (offset to address 0). The second command converts the presigned_image.hex file to binary format for signature injection.<br>
5. Compute the SHA\-384 hash of the executable code. The first command outputs the hash to the terminal, while the second saves the hash in binary format.<br>  
6. Inject the SHA\-384 digest of the executable code into the header binary file using update_header_value.py. The type_code argument (0x3) specifies that the data being injected is of type MDFU_PARTITION_HEADER_INTEGRITY. Refer to mdfu_partition_header.h for header type codes.<br> 
7. Compute the SHA\-384 hash of the header binary file and sign the hash using the private key stored in the keystore. The signature is saved in .der format.<br> 
8. Convert the signed header .der file to binary format using signing_tool.py.<br>
9. Combine the signature, header, and executable data binary files into a single signed binary file using bin_tool.py. Each file is injected at specific offsets:
...
2. In the command terminal, enter the following command to begin the firmware update:<br>
    ...
    ```
     pymdfu update --tool python-can-tp --can-tp-config ..\boot.X\config.toml --can-config ..\boot.X\config.toml --can-context pcan_500k_2M --client-id 161 --host-id 162 --image dist\default\production\signed_image.bin
    ```
    ...
    <br>pymdfu uses boot.X\config.toml to configure the CAN bus. By default, this demo uses the following configuration:

    * Nominal Bit Rate: 500 Kbps
    * CAN-FD: enabled (2Mbps)
    * CAN Frequency: 80 MHz

     Additional configuration values can be found in boot.X\config.toml. The required parameters depend on the hardware interface used to access the CAN bus. Additional details on each parameter can be found [here](https://python-can.readthedocs.io/en/stable/interfaces.html). 

    <br>**NOTE**: For a more verbose output detailing the packets being transferred, "--verbose debug" can be added to the command above. Additional details on the pymdfu command usage can be found [here](https://pypi.org/project/pymdfu/).</br>
...
_Figure 13. - Receive Upgrade Flow_<br>
...
_Figure 14. - Recover From Download Flow_<br>
...
_Figure 15. - Launch Executable Flow_<br>
...
_Figure 16. - Install Upgrade Flow_<br>
...
Two other states of note that occur as a result of the flows mentioned above are BOOTING and RESET
    * **BOOTING**<br>
        ...
        _Figure 17. - Booting Flow_<br>
        <br>
    * **RESET**<br>
        ![Reset Flow](./images/reset.jpg)<br>
        _Figure 18. - Reset Flow_<br>
...
_Figure 19. - Executable Memory Map_<br>
...
This specifies that the compiler should define an IVT table at address 0x80E200. It also instructs the C startup code to set the IVT base to this address. For more information about this option and other features for IVT definition, please refer to the XC-DSC compiler/linker documentation.  Refer to the device datasheet for alignment requirements for the IVTBASE register.
...
If porting this demo over to another dsPIC33A device, it is recommended that the device's default .gld file be used and the following modifications made. If the address values in the linker file are updated, ensure the associated values in mdfu_config.h are changed accordingly in both boot.X and app.X. Similarly, the values in postBuild.bat/.sh will need to be updated to reflect the changes made. See the [Application Post Build section](#application-post-build-details) for postBuild script details. 
...
* An __IVT_BASE definition has been added and set as 0x80E200. This is used later in the linker file to define the bootloader's interrupt vector table.
...
* Under Project Properties &rarr; XC-DSC &rarr; xc-dsc-ld, in the "Additional options" box, the option "--ivt=0x80E200" was added. This automatically generates a remapped interrupt vector table in XC-DSC **v3.20 or later** at the specified address. This address may need to be adjusted if using a different device.
...
![System Memory Map](./images/memoryMap.jpg)<br>
_Figure 20. - System Memory Map_<br>
...
As shown in _Figure 18_ the boot region resides in address range 0x800000-0x80CFFF (inclusive).  To configure PR0 to this range, we configure the PR0ST register to 0x000 and the PR0END to 0xC.  *NOTE* - the PRnST and PRnEND registers are offset from 0x800000 and are page aligned (the lower bits are hardware restricted to force page alignment).  These options are set in the configuration bits FPR0ST and FPR0END which are copied into the PR0ST and PR0END SFRs on reset.
...
As shown in _Figure 18_ the keystore region resides in address range 0x80D000-0x80DFFF (inclusive).  To configure PR1 to this range, we configure the PR1ST register to 0xD and the PR1END to 0xD.  *NOTE* - the PRnST and PRnEND registers are offset from 0x800000 and are page aligned (the lower bits are hardware restricted to force page alignment).  These options are set in the configuration bits FPR1ST and FPR1END which are copied into the PR1ST and PR1END SFRs on reset.
...
As shown in _Figure 18_ the executable region resides in address range 0x80E000-0x846FFF (inclusive).  To configure PR2 to this range, we configure the PR2ST register to 0xE and the PR2END register to 0x46.  *NOTE* - the PRnST and PRnEND registers are offset from 0x800000 and are page aligned (the lower bits are hardware restricted to force page alignment).  These options are set in the configuration bits FPR2ST and FPR2END which are copied into the PR2ST and PR2END SFRs on reset.
...
As shown in _Figure 18_ the download region resides in address range 0x847000-0x87FFFF (inclusive).  To configure PR3 to this range, we configure the PR3ST register to 0x47 and the PR3END to 0x7F.  *NOTE* - the PRnST and PRnEND registers are offset from 0x800000 and are page aligned (the lower bits are hardware restricted to force page alignment).  These options are set in the configuration bits FPR3ST and FPR3END which are copied into the PR3ST and PR3END SFRs on reset.
...
_Figure 21. - Authorized Recovery_<br>
...
## Porting to MPLAB Extensions for VS Code
MPLAB Extensions for VS Code has an ability to import an MPLAB X project. The application example project has a post-build script that handles post-processing of the compiled image for example signing. This is discussed in the #application-post-build-details section. The command line for executing that post-build uses features that differ between MPLAB X and MPLAB Extensions for VS Code.

Porting this project to MPLAB Extensions for VS Code will require manual modification to the project settings including, but may not be limited to, updating the pre/post build script calls and associated parameters.
