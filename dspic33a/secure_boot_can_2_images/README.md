...
2. In the command terminal, enter the following command to begin the firmware update:<br>
    ...
    ```
     pymdfu update --tool python-can-tp --can-tp-config ..\boot.X\config.toml --can-config ..\boot.X\config.toml --can-context pcan_500k_2M --client-id 161 --host-id 162 --verbose debug --image dist\default\production\signed_image.bin
    ```
