...
static uint8_t downloadMemory[MDFU_CONFIG_DOWNLOAD_LENGTH] __attribute__((address(MDFU_CONFIG_DOWNLOAD_ORIGIN), space(prog), noload));
#endif
...
static bool ArgumentsAreInvalid(uint32_t offset, uint32_t length, void const * const pointer)
{
    ...
    return  ((NULL == pointer) ||
            (offset >= MDFU_CONFIG_DOWNLOAD_LENGTH) ||
            (length > MDFU_CONFIG_DOWNLOAD_LENGTH) ||
            (endOffset > MDFU_CONFIG_DOWNLOAD_LENGTH));
} 