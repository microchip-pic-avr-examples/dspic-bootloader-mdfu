...
REM Fill in unimplemented memory locations in the executable image, calculate the CRC of the executable space, and place this value in the header 
...
REM Fill in unimplemented memory locations in the executable header, calculate the CRC of the executable header, and prepend this value to the header
...
REM Shift the image to address 0 and cut only the executable portion
hexmate r806000-81FFFFs-806000,"%projectDir%\%imageDir%\%imageName%" -O"%projectDir%\%imageDir%\bin.hex"

REM Create the .bin file for MDFU from the executable .hex file 
...
if exist "%projectDir%\%imageDir%\%unifiedImageName%" (
    REM Fill in unimplemented memory locations in the executable image, calculate the CRC of the executable space, and place this value in the header 
    hexmate r0-FFFFFFFF,"%projectDir%\%imageDir%\%unifiedImageName%" -O"%projectDir%\%imageDir%\%unifiedImageName%" -FILL=w1:0x00@0x806000:0x81FFFF +-CK=806080-81FFFF@806018w-4g5p814141AB

    REM Fill in unimplemented memory locations in the executable header, calculate the CRC of the executable header, and prepend this value to the header
    hexmate r0-FFFFFFFF,"%projectDir%\%imageDir%\%unifiedImageName%" -O"%projectDir%\%imageDir%\%unifiedImageName%" -FILL=w1:0x00@0x806000:0x81FFFF +-CK=806004-80607F@806000w-4g5p814141AB
)
