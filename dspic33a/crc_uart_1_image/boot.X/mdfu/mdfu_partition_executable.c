...
#ifndef EXECUTIVE_MEMORY_READ
#define EXECUTIVE_MEMORY_READ(destination, source, length) (void)memcpy(destination, source, length)
#endif
...
static size_t Read(uint32_t offset, size_t length, void *buffer)
{
    ...
    {
        EXECUTIVE_MEMORY_READ(buffer, &executableStart[offset], actualLength);
    }
...
/**
 * @brief    Copy not implemented for single partition bootloading.
 * @return   enum PARTITION_STATUS - Status indicating copy failed and is not implemented for single partition bootloading.
 */
static enum MDFU_PARTITION_STATUS Copy(const struct MDFU_PARTITION *source)
{
    /* Unused parameters */
...
static enum MDFU_PARTITION_STATUS Run(void)
{
     ...
     * user application.
     * Accessing 'ivtLocation' you will provide the address of the executable
...
static enum MDFU_PARTITION_STATUS ModeChange(enum MDFU_PARTITION_MODE mode)
{
    ...
    if (mode == (enum MDFU_PARTITION_MODE)(MDFU_PARTITION_MODE_READ & MDFU_PARTITION_MODE_WRITE))
    {
        ...
        status = flashRegion1.permissionsSet(permissions, false) ? MDFU_PARTITION_STATUS_SUCCESS : MDFU_PARTITION_STATUS_LOCKED;
    }
...
/**
 * @brief      Hash not implemented for CRC verification. 
 * @return     enum PARTITION_STATUS - Status indicating hash failed and is not implemented for CRC verification.
 */
static enum MDFU_PARTITION_STATUS Hash(enum MDFU_PARTITION_HASH_ALGORITHM algorithm, uint32_t offset, uint32_t length, uint8_t *digest)
{
    /* Unused parameters */
...
static bool ArgumentsAreInvalid(uint32_t offset, uint32_t length, void const * const pointer)
{
    ...
    return  ((NULL == pointer) ||
            (offset >= MDFU_CONFIG_EXECUTABLE_LENGTH) ||
            (length > MDFU_CONFIG_EXECUTABLE_LENGTH) ||
            (endOffset > MDFU_CONFIG_EXECUTABLE_LENGTH));
} 