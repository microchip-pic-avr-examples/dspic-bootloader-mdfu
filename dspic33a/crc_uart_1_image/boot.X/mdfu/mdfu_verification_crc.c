...
/**
 * @brief    Calculates the CRC of a specified range of an image
 ...
 * @param    uint32_t * const - pointer to the buffer to store the ouput CRC32Q value of the specified data.
 */
static bool PartitionCrcCalculate(struct MDFU_PARTITION const * const partition, const uint32_t startOffset, uint32_t length, uint32_t * const outputCrc)
{   
    uint32_t crc = 0;
    bool result = true;
    ...
    {    
        ...
        if(partition->read(startOffset+i, 1, &data) != 1)
        {
            result = false;
        }
 ...
 */
static enum MDFU_VERIFY_CODE VerifyHeader(struct MDFU_PARTITION const * const partition)
{
 ...
 *           no errors are detected.
 */
static enum MDFU_VERIFY_CODE VerifyPartition(struct MDFU_PARTITION const * const partition)
{
    ...
    if(MDFU_HeaderItemBufferRead(partition, MDFU_PARTITION_HEADER_INTEGRITY, sizeof(uint32_t), &expectedCrc) != sizeof(uint32_t))
    {
        headerError = true;
    }
    
    if(MDFU_HeaderItemBufferRead(partition, MDFU_PARTITION_HEADER_DATA_SIZE, sizeof(uint32_t), &codeSize) != sizeof(uint32_t))
    {
        headerError = true;
    }
    ...
    if( false == headerError )
    {
        ...
        if(PartitionCrcCalculate(partition, MDFU_CONFIG_CODE_OFFSET, codeSize, &calculatedCrc) == false){
            returnCode = MDFU_VERIFY_CODE_INVALID_INTEGRITY_CHECK;
        } else if(calculatedCrc != expectedCrc) {
            returnCode = MDFU_VERIFY_CODE_INVALID_INTEGRITY_CHECK;
        } else {
