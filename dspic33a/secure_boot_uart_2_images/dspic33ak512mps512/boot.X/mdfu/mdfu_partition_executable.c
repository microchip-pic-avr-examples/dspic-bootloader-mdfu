...
static uint8_t executableMemory[MDFU_CONFIG_EXECUTABLE_LENGTH] __attribute__((address(MDFU_CONFIG_EXECUTABLE_ORIGIN), space(prog), noload));
#endif
...
static enum MDFU_PARTITION_STATUS Hash(enum MDFU_PARTITION_HASH_ALGORITHM algorithm, uint32_t offset, uint32_t length, uint8_t *digest)
{
    ...
    if ((algorithm != MDFU_PARTITION_HASH_ALGORITHM_SHA2_384)) {
