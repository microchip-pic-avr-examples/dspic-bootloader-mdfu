...
#ifdef MDFU_CONFIG_DEDICATED_DOWNLOAD_PARTITION
#include "mdfu_partition_download.h"
#define UPDATE_PARTITION download
#else
#include "mdfu_partition_executable.h"
#define UPDATE_PARTITION executable
#endif
...
static void receiveExecuteTasks(void);
static void sessionNextStateLogic(void);
...
static bool TargetMemoryErased(void);

static uint8_t NextSeqNum = 0;
static enum MDFU_COMMAND_RESULT currentSessionState = MDFU_COMMAND_SESSION_WAITING;
static enum MDFU_COMMAND_RESULT nextSessionState = MDFU_COMMAND_SESSION_WAITING;

/**
 * @brief    Initializes the command processor layer.  
 * @return   None 
 */
void MDFU_CommandInitialize(void)
{
    NextSeqNum = 0;
    currentSessionState = MDFU_COMMAND_SESSION_WAITING;
    nextSessionState = MDFU_COMMAND_SESSION_WAITING;
...
static void receiveExecuteTasks(void)
{
    ...
    case MDFU_TRANSPORT_COMPLETE:
        handleTransportComplete(checkSequenceNumber(currentCommand.c_sequence), &currentCommand, &lastResponse, &errorResponse, &nextSessionState);
        break;
...
static void sessionNextStateLogic(void)
{
    ...
    if((currentSessionState == MDFU_COMMAND_SESSION_COMPLETE) || (currentSessionState == MDFU_COMMAND_SESSION_FAILED))
    {
        nextSessionState = MDFU_COMMAND_SESSION_WAITING;
    }
}
...
static enum MDFU_COMMAND_RESULT executeCommand(command_parser *command, response_parser *response)
{
    enum MDFU_COMMAND_RESULT result = MDFU_COMMAND_SESSION_FAILED;
    ...
    case MDFU_COMMAND_WRITE_CHUNK:
        if (MDFU_FirmwareUpdateWriteMemory(&UPDATE_PARTITION, command->dataPayload, command->dataLength) == MDFU_FIRMWARE_UPDATE_PROTOCOL_PASS)
        {
            result = MDFU_COMMAND_SESSION_IN_PROGRESS;
        }
        else
    ...
    case MDFU_COMMAND_GET_IMAGE_STATE:
        if (MDFU_Verify(&UPDATE_PARTITION) != MDFU_VERIFY_CODE_SUCCESS)
        {
...
static uint8_t mdfuGetClientInfo(uint8_t *clientInfo)
{
    ...
    clientInfo[2] = MDFU_PROTOCOL_VERSION_MAJOR;
    clientInfo[3] = MDFU_PROTOCOL_VERSION_MINOR;
    clientInfo[4] = MDFU_PROTOCOL_VERSION_PATCH;
...
static bool TargetMemoryErased(void)
{
    bool memoryErased = false;
    bool modeChanged = (UPDATE_PARTITION.modeChange(MDFU_PARTITION_MODE_READ & MDFU_PARTITION_MODE_WRITE) == MDFU_PARTITION_STATUS_SUCCESS);

    if (modeChanged)
    {
        memoryErased = ((MDFU_FirmwareUpdateEraseMemory(&UPDATE_PARTITION) == MDFU_FIRMWARE_UPDATE_PROTOCOL_PASS));
    }
