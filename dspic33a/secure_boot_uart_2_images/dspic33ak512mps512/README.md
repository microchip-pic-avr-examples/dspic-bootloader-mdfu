...
This specifies that the compiler should define an IVT table at address 0x80C200. It also instructs the C startup code to set the IVT base to this address. For more information about this option and other features for IVT definition, please refer to the XC-DSC compiler/linker documentation.  Refer to the device datasheet for alignment requirements for the IVTBASE register.
...
* Under Project Properties &rarr; XC-DSC &rarr; xc-dsc-ld, in the "Additional options" box, the option "--ivt=0x80C200" was added. This automatically generates a remapped interrupt vector table in XC-DSC **v3.20 or later** at the specified address. This address may need to be adjusted if using a different device.
...
As shown in _Figure 19_ the boot region resides in address range 0x800000-0x80AFFF (inclusive).  To configure PR0 to this range, we configure the PR0ST register to 0x000 and the PR0END to 0xA.  *NOTE* - the PRnST and PRnEND registers are offset from 0x800000 and are page aligned (the lower bits are hardware restricted to force page alignment).  These options are set in the configuration bits FPR0ST and FPR0END which are copied into the PR0ST and PR0END SFRs on reset.
...
As shown in _Figure 19_ the keystore region resides in address range 0x80B000-0x80BFFF (inclusive).  To configure PR1 to this range, we configure the PR1ST register to 0xB and the PR1END to 0xB.  *NOTE* - the PRnST and PRnEND registers are offset from 0x800000 and are page aligned (the lower bits are hardware restricted to force page alignment).  These options are set in the configuration bits FPR1ST and FPR1END which are copied into the PR1ST and PR1END SFRs on reset.
...
As shown in _Figure 19_ the executable region resides in address range 0x80C000-0x845FFF (inclusive).  To configure PR2 to this range, we configure the PR2ST register to 0xC and the PR2END register to 0x45.  *NOTE* - the PRnST and PRnEND registers are offset from 0x800000 and are page aligned (the lower bits are hardware restricted to force page alignment).  These options are set in the configuration bits FPR2ST and FPR2END which are copied into the PR2ST and PR2END SFRs on reset.
...
As shown in _Figure 19_ the download region resides in address range 0x846000-0x87FFFF (inclusive).  To configure PR3 to this range, we configure the PR3ST register to 0x46 and the PR3END to 0x7F.  *NOTE* - the PRnST and PRnEND registers are offset from 0x800000 and are page aligned (the lower bits are hardware restricted to force page alignment).  These options are set in the configuration bits FPR3ST and FPR3END which are copied into the PR3ST and PR3END SFRs on reset.
